...
transformed parameters{
  ...
  // according to threads, need to try what works without error
  ...
  for (c in 1 : C) {
    ...
    if (alpha[c, 3] > 0) {
      theta3[c] = inv_logit(lambda31 + theta3_b * alpha[c, 1]);
    } else {
      theta3[c] = inv_logit(lambda30 + theta3_b * alpha[c, 1]);
    }
...
model {
  ...
  //priors
  // Option 1: Use raw_nu_ordered ~ normal(0, 1) and softmax it (commented out)
  raw_nu_ordered ~ normal(0, 1);
  // Option 2: Directly define nu using a Dirichlet prior
