...
parameters {
  ...
  real<lower=0>theta3_b1;
  real<lower=0>theta3_b2;
...
transformed parameters{
  ...
  for (c in 1 : C) {
    ...
    // }
    if (alpha[c, 3] > 0) {
      theta3[c] = inv_logit(lambda31 + theta3_b1 * alpha[c, 1] + theta3_b2 * alpha[c, 2]);
    } else {
      theta3[c] = inv_logit(lambda30 + theta3_b1 * alpha[c, 1] + theta3_b2 * alpha[c, 2]);
    }
...
model {
  ...
  lambda30 ~ beta(5,20);
  lambda31 ~ beta(20,5);
  theta3_b1 ~ normal(0,2);
  theta3_b2 ~ normal(0,2);
